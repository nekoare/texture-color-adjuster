...
namespace TexColAdjuster
{
    public static class ColorAdjuster
    {
            ...
            float intensity, bool preserveLuminance)
        {
            ...
            // Calculate statistics for more accurate color matching
            var targetStats = CalculateLabStatistics(targetLab);
            var referenceStats = CalculateLabStatistics(referenceLab);
        ...
        private static Color[] HueShiftAdjustment(Color[] targetPixels, Color[] referencePixels, 
            float intensity, bool preserveLuminance)
        {
            // Calculate dominant hue from reference
            float dominantHue = CalculateDominantHue(referencePixels);
            float targetDominantHue = CalculateDominantHue(targetPixels);
            ...
            float intensity, bool preserveLuminance)
        {
            ...
            var targetStats = CalculateColorStatistics(targetPixels);
            var referenceStats = CalculateColorStatistics(referencePixels);
